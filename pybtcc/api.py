...
class btcc:
    def __init__(self, key, secret, options={}):
        ...
        if 'hostname' in options:
            ...
            self.hostname = 'data.btcchina.com'
        self.port = options['port'] if 'port' in options else 443
        self.market = options['market'] if 'pair' in options else 'btccny'
    ...
    def construct_url(self, call):
        ...
        if self.port != 443:
            base += ':%d' % (self.port,)
        return "https://%s/data/%s" % (base, call)
    ...
    def get_all_tickers(self, kind='auth'):
        params = {'market': 'all'}
        return self.api_request('tickers', params, kind=kind)
    ...
    def get_trades(self, limit=None, since=None, sincetype='id', kind='auth'):
        params = {'market': self.market}
        ...
        if limit is not None:
            if limit > 5000:
                ...
                params['limit'] = limit
        trades = self.api_request('historydata', params, kind=kind)
        return trades
